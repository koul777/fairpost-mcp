# 수용 기준 검증

검증일: 2026-09-07

| 기준 | 상태 | 근거 |
|---|---|---|
| AC-1 결정론 100회 | 통과 | `test_deterministic_for_100_runs` |
| AC-2 결정 경로 무네트워크 | 통과 | 코어 소켓 차단 테스트. 웹 기본 모드는 외부 요청이 없고, 사용자가 선택형 보강 스위치를 켠 경우에만 CSP `connect-src 'self'` 범위의 동일 출처 API를 호출 |
| AC-3 점수·등급·판정 금지 | 통과 | 출력 문자열 회귀 테스트 |
| AC-4 청년인턴 제외 | 통과 | 제외 후 유효 매칭까지 회귀 테스트 |
| AC-5 법령 참조 실재 | 통과 | 엄격 로더와 전체 규칙 검사 |
| AC-6 부재 문구 | 통과 | 출력 문자열 회귀 테스트 |
| AC-7 원문 offset·section | 통과 | CRLF, Unicode, NFKCㆍ제로폭 정규화 회귀 테스트 |
| AC-8 사전 교체 | 통과 | 임시 YAML 규칙 삽입 테스트 |
| AC-9 로컬 답변 저장 | 통과 | 격리 임시 경로 저장, 전체ㆍ조직별 privacy-safe purge, 마지막 답변 삭제 시 파일 제거 테스트 |
| AC-10 스키마 오류 실패 | 통과 | 잘못된 question severity 테스트 |
| AC-11 provenance | 통과 | 전체 규칙 검사 |
| AC-12 core 의존 방향 | 통과 | 정적 import 검사 |
| AC-13 웹·MCP 동일 출력 | 통과 | CRLF·Unicode 회귀 테스트와 학습 세트 2,310건 구조 해시 전수 비교 |
| AC-14 홀드아웃 격리 | 통과 | 실제 manifest 해시 중복 0 |
| AC-15 로컬 statute 금지 | 통과 | 로더 실패 테스트 |
| AC-16 시행일·해시 | 통과 | 6개 법령 스냅샷 검사 |
| AC-17 질문 발동근거 | 통과 | 질문 카드의 문맥ㆍoffsetㆍ섹션과 Python/웹 패리티 테스트 |
| AC-18 질문 관련성ㆍ점진적 공개 | 통과 | `review_scope` 검증, 공통ㆍ누락 슬롯 중복ㆍ후속 질문 기본 접힘, train-only 익명 집계와 보호 문맥 회귀 테스트 |
| AC-19 민간 반복감사 자동화 | 통과 | train-only 경로 사전검사, snapshot→audit→drift→로컬 review queue 단일 사이클, 복구 가능한 다중 파일 게시ㆍ일반 오류 rollback, 익명 출력과 경보 종료 코드 회귀 테스트 |
| AC-20 웹 사람 검토 메모 | 통과 | 질문별 담당자 답변ㆍ진행률ㆍ답변 포함 복사, 재분석ㆍ입력 지우기 시 메모리 답변 삭제, 공공기관 지정 유형ㆍ민간ㆍ규모별 적용 구분과 공식 근거, 브라우저 저장소 미사용. 선택형 보강을 켠 경우에만 동일 출처 API 호출 |
| AC-21 연결형 로컬 실행 | 통과 | 로컬 설정 파일의 명시적 로드·환경변수 우선, 웹·API·전체 MCP 동시 제공, 외부 Host/Origin 및 비공개 파일 접근 차단, 미활성화 POST 거부 |
| AC-22 실제 선택형 보강 연결 | 통과 | 2026-09-07 루프백 API 실호출: 공개 Korean Law MCP에서 현행 제7조 1건 조회 후 로컬 Ollama `qwen3:4b` 메모 생성 완료(45.9초). 실제 MCP 초기화·6개 도구 목록 확인. AI 추론 태그 제거·미완료 응답 거부 회귀 테스트 포함 |

AC-22는 합성 문구 `여성만 지원 가능`과 공기업·300명 이상 프로필을 사용한 연결
검증이며 법률 판단의 정확도 검증은 아닙니다. 연결 가능한 브라우저가 없어 이번
변경은 HTTP 실호출과 UI 회귀 테스트로 확인했고 화면 클릭 실증은 수행하지
않았습니다. Vercel 운영 환경의 AI 연결이나 배포 완료를 뜻하지 않습니다.

현재 소스 기준 자동 테스트 936개가 통과했습니다. `reports/build_artifact.json`의
893개 전수 JUnit SHA-256은 2026-08-31 릴리스 후보 산출물의 역사적 증거이며,
이번 변경의 새 릴리스 산출물로 오인하지 않습니다. 기본 MCP 전송 방식인
Streamable HTTP는 실제
로컬 포트에서 초기화, 도구 목록, 점검, 답변 저장ㆍ조회를 검증했습니다.
Vercel 운영 주소는 Bearer 인증 뒤에 원문을 저장하지 않는 읽기 전용 3도구만
허용합니다. 명시적 익명 모드도 같은 3도구와 요청 제한을 적용하지만 현재 운영에서는
비활성화했으며, 선택적 현행 법령 조회와 저장 도구를 포함한 6도구는 루프백 로컬에서만
허용하도록 검증합니다. 운영 규칙셋과 현재 로컬 규칙셋의 일치 여부는
`reports/vercel_deployment_audit.json`에 별도로 기록합니다.
정적 웹의 2026-07-26 기준선은 Chrome CDP로 예시 공고를 입력하고 점검한 뒤
1440×1000과 390×844 뷰포트에서 검증했습니다. 당시 두 화면 모두 문서 너비가 뷰포트와
일치했고, 입력ㆍ복사 버튼이 화면 안에 있으며, 결과 표시 후 빈 상태가
숨겨짐을 확인했습니다. 수치와 캡처 경로는
`reports/web_visual_audit.json`에 `historical`로 기록했습니다.
서버 없이 `web/index.html`을 직접 연 `file://` 실행도 같은 보고서에
기록했고, 전수 엔진 비교 결과는 `reports/web_engine_parity.json`에
원문 없이 저장했습니다.
2026-08-30 배포 후보는 `tools/verify_distribution.py`로 sdist와 wheel을
검사합니다. 실행에 필요한 코드ㆍ규칙ㆍ법령ㆍ웹 파일과
소스 재현 자료가 포함되고, 인증정보ㆍ사용자 답변ㆍ비공개 코퍼스 원문은
포함되지 않음을 `reports/distribution_audit.json`에 기록했습니다.
패키지에는 사용법을 보여 주는 합성 공고 예시가 있으므로 감사의
`contains_posting_text`는 `true`, 범위는 `synthetic_examples_only`로 정직하게
구분하고 실제 공고 원문은 `contains_real_posting_text: false`로 검사합니다.
격리된 임시 실행환경에서 wheel을 직접 설치해 소스 트리 밖에서도 규칙ㆍ조문 데이터 탐색,
CLI 시작과 AIㆍ건강 양방향 혼합 경계가 동작하는지 추가 확인했습니다.
질문 관련성 감사의 익명 집계와 재현 명령은
`docs/question-relevance-audit.md`와
`reports/question_relevance_audit.json`에 기록했습니다.
누락 슬롯 안에 접은 세 질문은 train 420건에서 총 1,260개
slot-question 관계를 비교해 불일치 0건을 확인했습니다. 최신 중첩 배치의
새 시각 캡처는 앱 내 브라우저 연결을 사용할 수 없어 보류했습니다.

현재 기본 사전은 법령 규칙 19개와 질문 카드 52개이며, 로컬 증거 14개는
현재 규칙셋ㆍ매칭 버전과 일치합니다. 재생성 입력이 없는 모델ㆍ맥락ㆍ사례
보고서 16개는 원래 정보를 보존한 `historical` 증거로 분리했으며 현재 성능 주장에
사용하지 않습니다. 민간 코퍼스 다양성 감사는 단일 출처와 직군 편중으로
`alert` 상태이고, v0.3에서는 정보성 게이트로 유지합니다.

## 미완료 품질 게이트

- G1 표현 탐지 정밀도: 봉인 홀드아웃 사람 라벨이 없어 미측정
- G2 부재 탐지 재현율·정밀도: 봉인 홀드아웃 사람 라벨이 없어 미측정
- PRD 정식 코퍼스: 공공 300건 + 민간 300건, 420/180 고정 분할,
  원문 해시 중복 0
- 정식 직군 분포: 현장 239ㆍ사무 306ㆍ기술 38ㆍ연구 17건
- 확장 코퍼스: 공공 300건 + 민간 3,000건, 2,310/990 고정 분할

코퍼스 수량 기준은 충족했지만 사람 라벨이 아직 없으므로 PRD의
정밀도·재현율 목표 달성은 주장하지 않습니다.
`tools/evaluate.py --enforce-targets`는 사람 라벨이 준비된 뒤 이 품질
게이트를 강제합니다. 현재 라벨링 화면의 해시와 실행 명령은
`reports/human_labeling_handoff.json`에 고정했습니다.
